...
def chamaPrevisaoPapelao(papelao, procentagem_treino, steps, n_future, n_epoca, nome_coluna):
    st.markdown(""" ### Aqui começa os dados da previsão do Papelao""")
    coluna = papelao[[nome_coluna]]
    coluna_dropada = coluna.dropna()
    st.markdown(""" > Tabela do papelao .dropna()""")
    ...
    # separa em treino e teste
    scaler = StandardScaler()
    df_scaled = scaler.fit_transform(coluna_dropada)
    ...
    ############################################################
    # pegar as datas de previsão
    dates = pd.to_datetime(papelao.index)
    predict_dates = pd.date_range(list(dates)[-1] + pd.DateOffset(1), periods=n_future, freq='b').tolist()
    # st.write(predict_dates)
    ############################################################
    # criar dataframe de previsão
